...
class MatterToDOMRenderer {
   ...
   */
  constructor(elem, { category, mask } = { category: 1, mask: -1 }) {
    this.elem = elem;
    ...
    this.body = Matter.Bodies.rectangle(initialBodyX, initialBodyY, width, height, { collisionFilter: { category, mask }});
...
export class Flipper {
  /**
   * @param {typeof Renderer} rendererCls
   * @param {HTMLElement} table
   */
  constructor(rendererCls, table) {
    /** @type {Matter.Engine} */
  ...
  flip() {
    ...
    const tableRenderer = new MatterToDOMRenderer(this.table);
    ...
    const ground = Matter.Bodies.rectangle(
      ...
      window.innerWidth / 2, bottom + (groundHeight / 2), window.innerWidth, groundHeight, { isStatic: true },
    );

    // add all of the bodies to the world
    Matter.Composite.add(this.engine.world, [...this.domRenderers.map(r => r.body), ground]);
 ...
 * @returns {HTMLElement[]}
 */
function getRelevantLeafNodes(elem) {
  return Array.from(elem.children).flatMap(c => c.children.length ? getRelevantLeafNodes(c) : c);
}
